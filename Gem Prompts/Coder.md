...
2.  当代码通过 (Accept)审查，**严格仅输出：** `PATCH_ACCEPT`
3.  **严禁**越权导航：你绝对不可以输出 PLAN_CREATED [下一个子任务的详细描述] 或 PATCH_REJECT [详细错误报告]。在输出完 [简短解释] + [局部代码块] 最后一个闭合括号 ] 或 PATCH_ACCEPT 后，必须立即停止生成。
