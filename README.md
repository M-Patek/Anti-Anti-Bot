...
任务分发器，管理任务列表，按顺序分发子任务。

PLAN_CREATED, TASK_COMPLETED_SUCCESSFULLY

Coder

代码实现者，执行任务，编写或修改代码，并响应 QA 的反馈。

局部代码块/Diff, PATCH_ACCEPT

QA

质量保证工程师，对照任务蓝图审查 Coder 的代码。

PATCH_ACCEPT, PATCH_REJECT

Auditor

全域领航员，负责代码审查流程的宏观导航和文件覆盖率（DFS 策略）。

NAVIGATOR_KICKOFF, TASK_FOR_VIGILANCE

Vigilance

性能与安全验证专家，根据 Auditor 的任务进行深度代码缺陷分析。
